...
public class StylesWriter
{
    ...
    /// <summary>
    /// Writes w:rPr for a style, using a subset of the run properties mapping.
    /// </summary>
    private void WriteStyleRunProperties(RunProperties props)
    {
        const string wNs = "http://schemas.openxmlformats.org/wordprocessingml/2006/main";

        // If nothing meaningful is set, skip rPr
        ...
        {
            _writer.WriteStartElement("w", "rFonts", wNs);
            _writer.WriteAttributeString("w", "ascii", wNs, props.FontName);
            _writer.WriteAttributeString("w", "hAnsi", wNs, props.FontName);
            _writer.WriteAttributeString("w", "cs", wNs, props.FontName);
            _writer.WriteEndElement();
        }
        ...
        if (props.IsBold)
        {
            _writer.WriteStartElement("w", "b", wNs);
            _writer.WriteEndElement();
        }
        if (props.IsBoldCs)
        {
            _writer.WriteStartElement("w", "bCs", wNs);
            _writer.WriteEndElement();
        }
        ...
        if (props.IsItalic)
        {
            _writer.WriteStartElement("w", "i", wNs);
            _writer.WriteEndElement();
        }
        if (props.IsItalicCs)
        {
            _writer.WriteStartElement("w", "iCs", wNs);
            _writer.WriteEndElement();
        }
        ...
        if (props.IsAllCaps)
        {
            _writer.WriteStartElement("w", "caps", wNs);
            _writer.WriteEndElement();
        }
        if (props.IsSmallCaps)
        {
            _writer.WriteStartElement("w", "smallCaps", wNs);
            _writer.WriteEndElement();
        }
        ...
        {
            _writer.WriteStartElement("w", "strike", wNs);
            _writer.WriteEndElement();
        }

        // Color (simple ICO index only; RGB is handled at run level)
        if (props.Color != 0)
        {
            ...
            if (colorHex != "auto")
            {
                ...
                _writer.WriteAttributeString("w", "val", wNs, colorHex);
                _writer.WriteEndElement();
            }
        }
        ...
        {
            _writer.WriteStartElement("w", "sz", wNs);
            _writer.WriteAttributeString("w", "val", wNs, props.FontSize.ToString());
            _writer.WriteEndElement();
        }
        if (props.FontSizeCs > 0 && props.FontSizeCs != 24)
        {
            _writer.WriteStartElement("w", "szCs", wNs);
            _writer.WriteAttributeString("w", "val", wNs, props.FontSizeCs.ToString());
            _writer.WriteEndElement();
        }
        ...
        {
            _writer.WriteStartElement("w", "highlight", wNs);
            _writer.WriteAttributeString("w", "val", wNs, ColorHelper.GetHighlightName(props.HighlightColor));
            _writer.WriteEndElement();
        }
        ...
        if (props.IsUnderline)
        {
            ...
            _writer.WriteAttributeString("w", "val", wNs, GetUnderlineType(props.UnderlineType));
            _writer.WriteEndElement();
        }
        ...
        {
            _writer.WriteStartElement("w", "vertAlign", wNs);
            _writer.WriteAttributeString("w", "val", wNs, "superscript");
            _writer.WriteEndElement();
        }
        else if (props.IsSubscript)
        {
            _writer.WriteStartElement("w", "vertAlign", wNs);
            _writer.WriteAttributeString("w", "val", wNs, "subscript");
            _writer.WriteEndElement();
        }
